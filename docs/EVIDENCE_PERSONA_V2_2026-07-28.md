...
**Статус: `--apply` выполнен заказчиком 2026-07-28 в 22:46 (+05).**

Проверка после деплоя (read-only):

| Что | Результат |
|---|---|
| `SOUL.md` в профиле | sha256 `3ab6c3c0…4972ef` = git, mode `444`, 23 430 байт |
| `agent.disabled_toolsets` | 12 записей, включая browser/file/delegation/session_search/image_gen/vision/tts/todo/clarify |
| Gateway | `active`, перезапущен 22:46:02 |
| Cron-промпты | утро 1 400 → 2 340 симв., вечер 700 → 1 405, 25 → 1 163, 27 → 978 |
| Trusted fingerprints | `imp04_refresh_cron_fingerprints.py` → «already current» (пересчитаны деплоем, mapping 0600 не менял состав) |
| Backend | `server.py`, `external_data.py` обновлены в `/opt/hermes-mariyam` |

### Откат — важно

Скрипт запускали дважды, поэтому бэкапов три:

| Бэкап | Содержимое | Годен для отката |
|---|---|---|
| `imp04-backup-20260728_222743` | SOUL 40 767, config 953, jobs 44 600 (без backend) | частично |
| **`imp04-backup-20260728_224538`** | полный снимок ДО деплоя (SOUL 40 767, config 953, jobs 44 600, оба backend-файла) | **да, это точка отката** |
| `imp04-backup-20260728_224600` | снимок УЖЕ задеплоенного состояния (SOUL 23 430, config 1 062) | нет — откат в него ничего не вернёт |

Правильная команда отката:

```bash
ssh timeagent@46.224.239.76 "bash /tmp/imp04/imp04_deploy.sh --rollback /home/timeagent/imp04-backup-20260728_224538"
```
...
На момент записи (22:50) сессий после рестарта ещё не было — первая появится
на утреннем cron 08:30 либо на первом сообщении Ойижон. Последние сессии ДО
деплоя (19:30): вход 21 611 и 22 398 токенов на 2 вызова, `cache_read` 24 320 и
23 552 — то есть кэш префикса закрывал около половины входа. После деплоя
префикс меньше, поэтому и кэшируемая часть станет меньше в абсолюте.

## 8. Решение заказчика по модели (2026-07-28)

Остаёмся на `gpt-5.6-luna` через n1n: у заказчика есть бонусный баланс.
Перевод на `deepseek/deepseek-chat` (fallback уже прописан) отложен —
вернуться к этому, когда бонусы закончатся: это главный оставшийся рычаг,
примерно ×5 по цене входа.
