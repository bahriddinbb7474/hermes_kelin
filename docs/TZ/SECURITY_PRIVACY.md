...
> **Наблюдаемое поведение (Hermes v0.18.2, зафиксировано Блоком 6И):** пользователь вне allowlist блокируется адаптером **до** agent session, LLM, tools и БД — `PASS_SECURITY`. Однако требуемый текст отказа `Кечирасиз, бу шахсий ёрдамчи.` **не отправляется** (тихий отказ). Результат теста: `FAIL_TEXT_AC / PASS_SECURITY`. До решения заказчика нормативное требование ТЗ (этот раздел) остаётся без изменений.
