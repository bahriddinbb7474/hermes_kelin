...
- Migration 003 реализована в repo: `monthly_budget_items` (user/month/category/item, planned quantity/unit/amount; минимум quantity или amount; unique item per month/category; composite FK на родительский `monthly_budget_plans`) и подготовленная для Stage 5.3A `monthly_plan_cycles` (status, household size, source, proposal/approval metadata; unique user/month).
