...
- **Stage 8 использует systemd timers как ops-контур:** запрет отдельного scheduler
  относится к диалоговой/бизнес-логике Hermes; ежедневный зашифрованный backup и
  heartbeat являются инфраструктурными задачами и не добавляют второй «мозг».
...
- После локальных gates статус Stage 5.3 = **CLOSED / LIVE PASS** ([evidence](../EVIDENCE_STAGE_5_3_LIVE_PASS_2026-07-23.md)). Repo
...
## Решение заказчика (2026-07-23) — Stage 5.3A cron identity

- В цикле 25/27/28/1 mutation разрешена только job шага «1 число»
  (auto-approve). Jobs 25/27/28 строго read-only; их private mapping entries не
  содержат mutating tools в `allowed_tools`.
- Cross-run idempotency — ответственность контракта `approve_monthly_plan` и
  unique constraints `monthly_plan_cycles`, не identity/Stage 5.3 guard.
- `mariyam_identity_guard` **repo 1.1.0 / deployed 1.0.4**: repo содержит
  fail-closed cron resolver с binding session/job/fingerprint/prompt/private
  mapping/tool allowlist; Telegram policy 1.0.4 сохранена regression suite.
- Production cron jobs, VPS mapping, `approve_monthly_plan`, backend/БД и deploy
  этим решением не создаются. Inventory остаётся **21**.

## Решение заказчика (2026-07-24) — Stage 5.3A вариант A (разблокировка imp02)

Причина: STOP-отчёт imp02 показал, что draft-строку `monthly_plan_cycles`
(`waiting_oyijon`) и переход в `waiting_admin` не создаёт ни один существующий
безопасный путь, а `approve_monthly_plan` пишет только terminal-статусы. Это
делало ТЗ п.1/п.3 (jobs 25/28) нереализуемыми при «25/27/28 строго read-only».

Утверждён **вариант A**: уточнение решения от 2026-07-23.

- Добавляется единственная узкая cycle-мутация — user-scoped tool
  `open_monthly_plan_cycle` (`action=open|escalate`). Он пишет **только** строку
  `monthly_plan_cycles`; никаких прав на `monthly_budget_plans`/
  `monthly_budget_items`/`transactions`.
- Правило «25/27/28 read-only» уточняется: **jobs 25 и 28 могут выполнять только
  свою узкую cycle-мутацию** (`open` и `escalate` соответственно) через private
  mapping allowlist; расход/transaction-мутации им по-прежнему запрещены.
  **Job 27 остаётся строго read-only.** Job 1 — read-only + `approve_monthly_plan`.
- Repo tool count 22 → **23**; deployed остаётся **21** (deploy — задача imp02).
- Контракт/поведение `approve_monthly_plan` не меняется.

## Решение заказчика (2026-07-25) — расширение `open_monthly_plan_cycle(open)`

Причина: job 25 имеет allowlist «read-only + `open`», но `open` требовал уже
существующий budget-draft (иначе `EMPTY_DRAFT`), а `set_monthly_budget` в job 25
не допущен. Чтобы job 25 мог создать draft без права на `set_monthly_budget`:

- `open` расширяется: если у Ойижон ещё **нет** budget-draft на плановый месяц,
  backend **детерминированно вычисляет и персистит** draft будущего месяца из
  источников ТЗ 5.3A п.1 — последний approved plan (приоритет как прежнее явное
  решение) ∪ категории из фактических расходов за 3 последних месяца по их
  округлённому среднему. Сумм не выдумывает. Обязательные платежи —
  фича Stage 6 (`recurring_obligations`), пока не реализована → не включаются.
- `open` пишет только `monthly_budget_plans` (сгенерированный draft) и строку
  `monthly_plan_cycles`; `monthly_budget_items`/`transactions` не трогает.
- Нет источника (ни approved plan, ни расходов) → `NO_PLAN_SOURCE`, без мутации.
  Уже существующий zero-sum plan не перезаписывается → `EMPTY_DRAFT`.
- `set_monthly_budget` в allowlist job 25 **не добавляется**. Контракт
  `approve_monthly_plan` не меняется. Tool count остаётся 23.

## Решение заказчика (2026-07-25) — read-tool статуса + 5 cron jobs (imp02)

- Добавляется read-only tool `get_monthly_plan_cycle(user_id, month)` (repo count
  23 → **24**): jobs 27/28/1b гейтят поведение по фактическому статусу цикла
  (без него job 27 read-only не мог знать, одобрен ли план). Мутаций нет.
- Gap 2 (один job = один `--deliver`): job «1 число» разбивается на **1a**
  (delivery Ойижон, `approve_monthly_plan(source=auto)`) и **1b** (delivery админ,
  read-only fallback: при `NO_PLAN_SOURCE`/отсутствии approve — одно уведомление).
  Итого **5 production jobs: 25, 27, 28, 1a, 1b**.
- Allowlists (private cron mapping): 25 → read-only + `open_monthly_plan_cycle`;
...
## Решение заказчика и реализации (2026-07-29) — ритм дня fix02

- Пять намаз-напоминаний — короткие check-in сообщения из pool по 10
  кириллических шаблонов на слот. Выбор зависит от даты и не повторяет
  вчерашний вариант. Бомдод начинается полным саломом и содержит
  «Намоз уйқудан афзал».
- Ежедневный systemd-планировщик читает свежий Aladhan-кэш и создаёт пять
  finite one-shot за 10 минут до намаза плюс отдельный список времён в 07:45.
  Все шесть jobs используют `no_agent=true`; пустой stdout означает
  детерминированную тишину и не создаёт отложенного дубля.
- Утренний LLM-тик переносится с 08:30 на 08:00 и содержит только приветствие,
  заботливую погоду без указаний и 1–2 разговорные новости. Намаз и
  obligations вынесены в отдельные механизмы. Вечерний 19:30 заканчивается
  единственным вопросом о планах на завтра.
- Новости загружаются из `backend/news_sources.json`, а не из SOUL:
  УзА удалён, Кун.уз сохранён; предложены Новости ООН, Дойче Велле и
  Евроньюс. Topic/source являются optional аргументами существующего
  `get_daily_news`, поэтому tool inventory остаётся 29. Заказчик утвердил
  этот точный список и deploy 2026-07-30.
- «сплю»/«ухлаяпман» включает quiet-state до 08:00 следующего утра;
  «Қуръон ўқияпман» — на 90 минут. В 20-минутные окна от начала каждого
  намаза non-critical no-agent output пустой, а обычный cron delivery получает
  штатный `[SILENT]`. Health plugin остаётся раньше quiet-gate и его прямая
  доставка админу не блокируется.
- До controlled deploy заказчику были показаны и им утверждены два шаблона на
  каждый слот и точный список RSS. Deploy и live-smoke завершены 2026-07-30
  только на тестовых identity; реальные аккаунты не подключались.

## Решение заказчика и реализации (2026-08-02) — пользовательские RSS/Atom (imp11)

- Ойижон может через один owner-bound tool добавлять, отключать и перечислять
  свои ленты; администратор имеет только narrow cross-target доступ через
  identity guard. Уведомления администратору при этих действиях запрещены.
- Migration 006 хранит источник и его темы одной строкой. Стандартные ленты
  остаются строгим repo-конфигом, а `get_daily_news` объединяет их только с
  активными лентами владельца.
- До любого запроса разрешены только HTTPS URL на публичных адресах. Каждый
  redirect повторно валидируется, соединение привязано к проверенному IP для
  защиты от DNS rebinding; RSS/Atom обязателен, timeout 12 секунд, ответ не
  более 2 MiB, не более 15 активных лент на пользователя.
- Runtime tool count = **30/30/30**, identity guard = **1.4.0**. Полный pytest:
  **339 passed, 88 skipped, 0 failed**.
- Controlled deploy: сначала `mariyam_test`, затем семейный
  `mariyam_oyijon`; backup создан, migration 006 применена, перезапущен только
  семейный gateway. Telegram add/list/disable/re-enable PASS, в семейной БД
  оставлена одна полезная активная технологическая лента, admin-alerts = 0.
- SOUL в imp11 не менялся: точная предлагаемая правка §9 передана Terra в
  `tasks/sol/imp11.report.md`, чтобы не пересечься с её параллельной работой.
