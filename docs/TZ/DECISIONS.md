...
Исполнять только `TZ_Hermes_Mariyam_FINAL_v3_0.md` (внутри — версия **3.14**, раздел 0.1–0.14 = changelog): §0.9 — Stage 5.1 CLOSED / LIVE PASS; §0.10 — planned household finance; §0.11 — первая Stage 5.2 offline implementation; §0.12 — Stage 5.2 live FAIL и reference prices; §0.13 — SKILL/test fix; §0.14 — deterministic SOUL/effective-prompt fix и новый offline PASS. Старые v1/v2/review-файлы не использовать как рабочие требования.
