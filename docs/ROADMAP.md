...
| 5.1 | Аналитика расходов + месячный план (quantity/unit, compare/trend, plan/fact, осторожные советы) | ⬜ **PLANNED / NOT STARTED** (ТЗ v3.7 §0.7). Target tools **21**. Live-тесты **запрещены** до закрытия self-improvement SKILL drift. |
...
- **Этап 5.1 (аналитика + monthly plan)** — **NOT STARTED** (ТЗ v3.7); target tools 21; live 5.1 blocked until SKILL drift fix.
- **КРИТИЧЕСКИЙ ОТКРЫТЫЙ БЛОКЕР:** self-improvement/curator **изменил runtime SKILL.md** после E2E msg4 (sha drift; служебное сообщение в Telegram). SKILL восстановлен (`dfc7e327…`). **Следующие live-этапы (включая 5.1 live) и handover запрещены** до минимального фикса.
