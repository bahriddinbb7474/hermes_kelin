...
| 6 | Hermes cron: reminders, recurring obligations, утро/вечер, новости, погода, намаз | 🟡 **PARTIAL — шаг 1:** migration 005 + recurring obligations storage/tools реализованы; inventory **26/26/26**, identity guard **1.3.0**; paid не создаёт transaction, next due вычисляет backend по approved rule. Hermes cron reminders и остальные функции Stage 6 — следующая задача. |
...
- **Stages 5.3A–6:** Stage 5.3A approval runtime/production cron active; Stage 6 obligation backend step реализован как migration 005 + tools 25–26. Migration 004/utility connector остаются NO-GO; Stage 6 reminder cron и утро/вечер/новости/погода/намаз ещё не реализованы.
