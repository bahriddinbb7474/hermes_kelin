...
| 6 | Hermes cron: reminders, recurring obligations, утро/вечер, новости, погода, намаз | 🟡 **PARTIAL / LIVE DEPLOYED (2026-07-26):** inventory **29/29/29**, health PASS, Aladhan/Hanafi + UzA/Kun + cache PASS; 08:30/09:15/19:30 jobs active, private mapping 0600 = 8/8, post-fix cron trust resolve PASS. Остаток: private OpenWeather key отсутствует; первый scheduled tick fail-closed до prompt-normalization; due-now one-shot не доставлен из-за provider HTTP 524; manual sensitive replay не разрешён safety gate. Evidence `EVIDENCE_STAGE_6_CRON_2026-07-26.md`. |
