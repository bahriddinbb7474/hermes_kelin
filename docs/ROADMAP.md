...
| 5.3A | Approval cycle 25/27/28/1, `approve_monthly_plan`, deterministic cron identity | 🟡 **PARTIAL**: identity plugin **1.1.0** deployed (offline/live fail-closed probes + Telegram regression PASS); **backend tools `approve_monthly_plan` + `open_monthly_plan_cycle` + `get_monthly_plan_cycle` реализованы в repo — repo count 24; imp02 controlled deploy на VPS поэтапно (23→24)**, детерминированные state machine + идемпотентность на schema migration 003; production cron = 5 jobs (25/27/28/1a/1b), создание mapping/jobs — в процессе imp02 |
