...
| 5.3 | Семейный/product plan, product quantities/amounts, last/weighted average/manual reference price | ✅ **OFFLINE PASS / LIVE PENDING (v3.18)**; migration 003 active на VPS; repo добавляет optional read-only lookup в существующий status tool и profile-only запрет `terminal` + `code_execution`; tools остаются 21; controlled fix deploy/E2E pending |
