...
| 3 | Голос: сквозной STT-тест (голос→STT→LLM→БД, числа ≥90%), бюджет; TTS отложен (v3.2) | ✅ **CLOSED / AC PASS (2026-07-29):** `gpt-4o-transcribe` через OpenAI API, local faster-whisper fallback; 3 модели × (15 real + 20 expense proxy + 7 medical proxy), 126/126 API success; final medical 7/7; строгий voice→DB 18/20 (90%), суммы 19/20 (95%); прогноз при 20 voice/день ≈ USD 0.667/мес |
...
| 6 | Hermes cron: reminders, recurring obligations, утро/вечер, новости, погода, намаз | ✅ **CLOSED / LIVE PASS на test-user; pre-handover reliability CLOSED (2026-07-28):** inventory **29/29/29**, health PASS; OpenWeather + Aladhan/Hanafi + UzA/Kun + cache 0600 PASS. Normal ticks 08:30/09:15/19:30 completed `ok`; manual morning digest и one-shot доставлены. Imp08 добавил active +15-minute watchdog для 8 critical jobs (single retry → direct test-admin alert) и provider-independent `no_agent` one-shot. Live: retry delivery, retry-fail alert, healthy silence, exact no-agent Telegram delivery с LLM sessions=0; cleanup и mapping 9/9 PASS. Evidence `EVIDENCE_CRON_WATCHDOG_2026-07-28.md`. |
...
- **Stages 5.3A–6:** Stage 5.3A approval runtime/production cron active; Stage 6
...
- **Pre-handover cron reliability:** user-systemd timer active/enabled с
  existing linger; 8 watched jobs остаются trusted `script=null`,
  `no_agent=false`. Private SQLite claim гарантирует максимум один retry;
  ambiguous/interrupted/retry-fail state эскалируется test-admin без LLM.
  User one-shot остаётся untrusted и исполняется private `no_agent` script.
