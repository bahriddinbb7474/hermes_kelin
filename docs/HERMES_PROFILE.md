...
> **Историческая справка (пометка imp07, 2026-08-02).** Ниже — чек-лист и
> статусы на июль 2026. Цифры и SHA в этом файле устарели: инструментов 30 (не
> 21), Stage 5.3A–8 закрыты, mutable `SKILL.md` упразднён — канон один,
> `deploy/hermes_profile_mariyam_oyijon/SOUL.md`. Актуальные процедуры —
> `deploy/DEPLOY.md`, актуальные статусы задач — `tasks/REGISTRY.md`,
> актуальный SHA SOUL — `EXPECTED_SOUL_SHA256` в
> `tests/test_mariyam_skill_protection.py`. Файл сохранён как история приёмок,
> выполнять по нему установку с нуля нельзя.
...
- Stage 5.2 закрыт: Message 1/2 подтверждены live, canonical SOUL установлен,
  ...
  при API calls = 0. Новый платный Telegram/provider test не выполнять.
