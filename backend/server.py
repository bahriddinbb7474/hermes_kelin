...
TOOLS = [
    ...
    ("set_monthly_budget", "Создать/обновить план категории и атомарно заменить optional product items. Для подтверждённого товара используй exact fields item_name_normalized, item_name_display, planned_quantity, unit, planned_amount_uzs, reference_unit_price_uzs, price_basis, price_as_of; never retry with items=[] после product validation error", schema({**pick("user_id", "month", "category_code", "planned_amount_uzs", "note"), "items": P["budget_items"]}, ["user_id", "month", "category_code", "planned_amount_uzs"])),
